...
(function () {
  ...
  // If running on OpenShift, override the port *and* hostname
  // because there we cannot bind to all interfaces, *and* the hostname
  if (process.env.OPENSHIFT_APP_NAME) {
    port = process.env.OPENSHIFT_NODEJS_PORT;
    options.host = process.env.OPENSHIFT_NODEJS_IP;
...
app.connect = function (callback) {
  ...
  store.connect(function (err) {
	...
	app.listen(options.port, options.host);	// Host might be undefined unless running on OpenShift
